...
void GameManager::loadResources() {
  if (!font.loadFromFile(
          "/home/tridung/op/codeGame/Roboto-Black (1).ttf")) {
    cerr << "Failed to load font from backup path!" << std::endl;
    throw std::runtime_error("Font loading failed");
  }

  if (!playerBattleTexture.loadFromFile("/home/tridung/op/codeGame/d6d2c141-b294-46f8-a02a-824409126f90.jpg")) {
  ...
  if (!enemyBattleTexture.loadFromFile(
          "/home/tridung/op/codeGame/eb444671-2fc9-44ca-acf2-4c33f6cd796e.jpg")) {
    ...
    if (battleSystem.isBattleOver()) {
      if (battleSystem.isPlayerVictorious()) {
        ...
        if (enemy) {
          ...
          cout << "Gained " << expGained << " experience points!" << std::endl;
...
void GameManager::drawInventoryScreen() {
  ...
  std::string eqName = (eq != SkillType::None) ? getSkillData(eq).name : "None";
...
void GameManager::drawLoadSlotMenu() {
  ...
  for (int i = 0; i < 3; ++i) {
    bool exists = std::find(slots.begin(), slots.end(), i + 1) != slots.end();
    std::string label =
        "Slot " + std::to_string(i + 1) + (exists ? "" : " (empty)");
    sf::Text slotText(label, font, 32);
    if (!exists)
...
void GameManager::drawBattleResultPopup() {
  ...
  if (lastBattleWon) {
    ...
    std::string txt = "Congratulations! You won the battle.\n";
    txt += "You gained " + std::to_string(lastBattleExp) + " EXP.\n";
...
vector<int> GameManager::getExistingSaves() const {
  vector<int> slots;
  for (int i = 1; i <= 3; ++i) {
    ifstream file("save" + std::to_string(i) + ".txt");
